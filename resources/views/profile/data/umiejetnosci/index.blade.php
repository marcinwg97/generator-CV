...
<td><button class="btn btn-warning"><a href="{{ route('profile.data.umiejetnosci.edit', $umiejetnoscis->id) }}">Edytuj</a></button></td>
<td><form action="{{ route('profile.data.umiejetnosci.destroy', $umiejetnoscis->id) }}" method="post" accept-charset="utf-8">
@csrf
@method("DELETE")
...
<button class="btn btn-success"><a href="{{ route('profile.data.umiejetnosci.create') }}">Dodaj</a></button>
@endsection